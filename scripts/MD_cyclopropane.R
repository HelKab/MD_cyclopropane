...
d = read.table("results/MD_cyclopropane.dat", header=T)
par(mgp=c(2,1,0))
...
saveGIF(expr = anim(), interval=.1, outdir = getwd(), movie.name="results/anim_MD_forces.gif")
...
saveGIF(expr = anim(), interval=.1, outdir = getwd(), movie.name="results/anim_MD_traces.gif")

png("results/Energies_MD.png")
inter = 0:500
plot(d$t[inter],d$E_tot[inter],type="l",xlab="t (ps)",ylab="Energy (kcal/mol)",col="black", ylim=c(0,max(d$E_tot)))
